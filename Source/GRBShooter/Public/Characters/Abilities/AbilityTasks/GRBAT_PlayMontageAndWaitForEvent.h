...
#include "GRBAT_PlayMontageAndWaitForEvent.generated.h"

class UGRBAbilitySystemComponent;

/** Delegate type used, EventTag and Payload may be empty if it came from the montage callbacks */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FGRBPlayMontageAndWaitForEventDelegate, FGameplayTag, EventTag, FGameplayEventData, EventData);

/**
 * 将 PlayMontageAndWait 和 WaitForEvent 结合在一起的复合异步节点.
 ...
 */
UCLASS()
class GRBSHOOTER_API UGRBAT_PlayMontageAndWaitForEvent : public UAbilityTask
{
	GENERATED_BODY()
...
private:
	...
	FGRBPlayMontageAndWaitForEventDelegate OnCompleted;

	/** The montage started blending out */
	UPROPERTY(BlueprintAssignable)
	FGRBPlayMontageAndWaitForEventDelegate OnBlendOut;

	/** The montage was interrupted */
	UPROPERTY(BlueprintAssignable)
	FGRBPlayMontageAndWaitForEventDelegate OnInterrupted;

	/** The ability task was explicitly cancelled by another ability */
	UPROPERTY(BlueprintAssignable)
	FGRBPlayMontageAndWaitForEventDelegate OnCancelled;

	/** One of the triggering gameplay events happened */
	UPROPERTY(BlueprintAssignable)
	FGRBPlayMontageAndWaitForEventDelegate EventReceived;

public:
	// Constructor and overrides
	UGRBAT_PlayMontageAndWaitForEvent(const FObjectInitializer& ObjectInitializer);
	...
	static UGRBAT_PlayMontageAndWaitForEvent* PlayMontageAndWaitForEvent(
